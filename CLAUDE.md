...
- 管理者専用パス: `src/hooks.server.ts` の `ADMIN_ONLY_PREFIXES`（`/database/accounts`, `/settings/integrations`, `/settings/email`, `/api/accounts`, `/api/integrations`, `/api/email/settings`, `/api/documents/test`）は `event.locals.account.permission !== 'admin'` の場合にアクセスを拒否する（`/api/*` は403 `errors.forbidden()`、ページは `/` へ303リダイレクト）。`/api/email/send`（`send_email` MCPツール、全ユーザーが利用）は対象外。サイドバーの「アカウント」リンク・設定サブナビの「API連携」「メール」リンクも `data.account.permission === 'admin'` の場合のみ表示する。`/api/documents/test`（`/ui`デモページ用）は本番環境でも一般ユーザーがR2にテスト文書を生成できないようadmin専用とする
