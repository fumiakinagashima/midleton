...
### v2以降（TODO）: ワークフロー生成
一般的なノーコードツールでは「データモデル（CRUD）」と「トリガー→アクションのワークフロー」がセットで提供される。Midletonでもアプリ生成とセットで後続実装する。

**Why:** ユーザーから、CRUD生成だけでは一般的なノーコードツールの体験に対して片手落きという指摘。まずCRUD生成を作り、ワークフローは設計・実装をTODOとして残す（2026-06-12）。
...
- [x] 完了通知の方式を段階的に検討
  ...
  - [ ] 通知センター（汎用）を追加する: 非同期ジョブ・通知は今後も増える想定のため、ポーリングカード（その場での即時表示用）に加えて永続的な通知の仕組みを用意する
    - D1に`notifications`テーブル（type, title, jobId/result, read状態, createdAt等）を追加し、ジョブ完了時にKVに加えて記録する
    - 通知センターはタイトルのみの一覧表示。未読件数は軽量な一覧取得ポーリングで表示する（ジョブ単位のポーリングより低コスト）
    - クリックすると新規チャットが開き、その通知内容をシードにAIとの会話が始まり、そこから次の操作に繋げられる
