...
- [ ] Webカメラでの名刺スキャン機能（QRスキャナー風 UI）
- [ ] OpenCV.js（`@techstark/opencv-js`、動的 import で遅延ロード）によるリアルタイム名刺枠検出
- [ ] 検出枠が安定したら自動撮影 → 透視変換で歪み補正・トリミング → `/api/bizcard` に送信
- [ ] 手動シャッターボタン（自動検出が機能しない場合のフォールバック）
- [ ] カメラ権限拒否・カメラ非搭載・OpenCV読み込み失敗時はファイルアップロードに自動フォールバック
- [ ] 連続スキャン対応（撮影後にカメラビューへ復帰、ストリーム再利用）
