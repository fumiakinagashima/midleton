...
### ① ストリーミングレスポンス
- [ ] `/api/chat` を SSE（Server-Sent Events）ストリーミングに変更
- [ ] フロントでテキストをトークン単位でタイプライター表示
- [ ] UI コンポーネント（Form / Table 等）はストリーム完了後に描画
- [ ] ストリーミング中の中断・エラー処理

### ② チャット UI ブラッシュアップ
- [ ] ユーザー / AI メッセージのレイアウト・デザイン刷新
- [ ] サジェストプロンプトチップ（初期画面・入力欄）
- [ ] AI コンポーネントのフェードイン・スライドアニメーション
- [ ] タイポグラフィ・余白・カラーの改善

### ③ 新 AI レスポンスコンポーネント
- [ ] `Stats` コンポーネント（大きな KPI 数値＋トレンド矢印、複数並列表示）
- [ ] `Pipeline` コンポーネント（商談ステージの横並びカンバン風表示）
- [ ] `Timeline` コンポーネント（活動履歴の時系列ビジュアル）
- [ ] システムプロンプトに新コンポーネントの仕様を追加
- [ ] デモ用ユースケース確認（「今月の商談状況は？」→ Stats + Pipeline）

### ④ 名刺画像取り込み
- [ ] チャット入力欄に画像アップロードボタン追加
- [ ] multimodal メッセージ（テキスト + 画像）を Claude API に送信
- [ ] 名刺から氏名・会社名・電話番号・メール・役職・住所を JSON 抽出するプロンプト設計
- [ ] 抽出結果を顧客／担当者登録 Form に pre-fill して表示
- [ ] トークン消費量・精度の検証（日本語名刺対応確認）
