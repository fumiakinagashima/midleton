...
- [x] `/database/[type]` 一覧上部に検索バーを追加（クライアント側。表示中の列の表示値への部分一致）
- [x] 列ヘッダクリックでソート（number型は数値比較、他は `localeCompare('ja')`）
- [x] 検索クリア・ヒット件数表示。`Search`/`X` アイコン
...
- [x] チャットの役割を「SELECTのみ（読み取り＋動線ボタン）」に明確化。登録・編集・削除は固定ダイアログ経由に限定
- [x] 書き込み系ツール（`create_*`/`update_*`/`delete_*`）をストリーミングAIのツール一覧から除外
- [x] AIが返す `<ui type="form">` は動線ボタンとして表示（自動でダイアログを開かず、ユーザーが押して開ける）
- [x] チャットのターン単位UI再設計: 「直前の1往復のみをメイン表示 + 過去はドロワー」構成
- [x] コア4エンティティの詳細/編集/登録を共通の中央ダイアログ（`RecordDialog`）に統一

### 6. メモ・議事録からの連続登録（`feature/v3-note-to-records`）
- [x] 商談メモ等を貼ると、AIが内容を要約し登録対象ごとに `<ui type="form">` 動線ボタンを並べる（prefill値入り）
...
- [x] 企業新規登録後に担当者登録する場合、登録した企業のIDを担当者の customerId にプリセット
- [x] チャット内の名刺コンポーネントもダイアログ化（portal action で position:fixed の重なり問題を修正）
