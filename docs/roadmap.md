...
- [ ] テーブル作成・フィールド編集を中央ダイアログ（`SchemaEditorDialog`）に統一。現在フルページ遷移になっている `/database/[type]/schema` をダイアログ化し、一覧ページを離れずにスキーマ編集できるようにする
- [ ] コアテーブル（customers/contacts/deals/activities）もカスタムカラムの追加・編集・削除をダイアログで行えるようにする
- [ ] カスタムテーブルの新規作成（テーブル名・フィールド定義）もダイアログで完結させる
